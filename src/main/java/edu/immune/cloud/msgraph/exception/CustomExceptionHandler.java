...
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMissingServletRequestParameter(MissingServletRequestParameterException ex,
		...
		String error = ex.getParameterName() + " mandatory parameter is missing";
		return new ResponseEntity<Object>(new ErrorResponse<>(Arrays.asList(error)), HttpStatus.BAD_REQUEST);
	}

	/**
	 * Custom exception handler for the Exception thrown when
	 * {@link org.springframework.validation.annotation.Validated} is used in controller.
	 * 
	 * @param ex
	 * @param request
	 * @return
